...
const fakeArticlesData = [
  ...
  },
  {
    ...
    preamble:
      "Før NM-sprinten i fristil i Grova i Meråker, fikk Even Northug gode råd av storebror Petter.",
  },
];
const PORT = 6010;
...
function requestHandler(req, res) {
  ...
  if (Math.random() > 0.1) {
    if (req.url === "/articles/sports") {
      content = {
        articles: fakeArticlesData.filter((a) => a.category === "sport"),
      };
      status = 200;
    } else if (req.url === "/articles/fashion") {
      content = {
        articles: fakeArticlesData.filter((a) => a.category === "fashion"),
      };
      status = 200;
    } else {
    ...
    content = {
      message: "Ooooops! this shouldn't be shown to clients probably...",
    };
    status = 500;
  }
